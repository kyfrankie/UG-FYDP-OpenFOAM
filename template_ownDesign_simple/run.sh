...
AOA=("-AOA6")
list=("internalField  uniform (15.91235033 1.67245541 0.0 );" )
liftDir=("liftDir           (-0.10452846 0.9945219 0.0 );")
dragDir=("dragDir           (0.9945219 0.10452846 0.0 );")

# AOA=("-AOA4" "-AOA6")
# list=("internalField  uniform (15.96102480 1.11610358 0.0 );" "internalField  uniform (15.91235033 1.67245541 0.0 );" )
# liftDir=("liftDir           (-0.06975647 0.9975641 0.0 );" "liftDir           (-0.10452846 0.9945219 0.0 );")
# dragDir=("dragDir           (0.9975641 0.06975647 0.0 );" "dragDir           (0.9945219 0.10452846 0.0 );")
...
for file in ../mesh/*.msh
  ...
  for u in "${list[@]}"
    do
      ...
      echo "RUN: pimpleFoam"
      mpirun -np 40 simpleFoam -parallel  2>&1 | tee -a logs/solver/pimpleFoam.log
      mpirun -np 40 simpleFoam -parallel -postProcess -funcs "(forceCoefficient yPlus CourantNo wallShearStress force Q vorticity)" 2>&1 | tee -a logs/solver/postProcess.log
      ...
      #tar -czvf "../result/$name/$name${AOA[t]}.tar.gz" .
      tar -czvf "../result/$name/$name${AOA[t]}.tar.gz" ./postProcessing/ ./logs/
